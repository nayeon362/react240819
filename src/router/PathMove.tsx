...
export default function PathMove() {
    ...
    // - a요소로 경로이동을 하게되면 브라우저가 새로운 요청을 보냄
    // - 현재 페이지의 상태가 유지되지 않음

    // <Link>:
    // - 현재 웹 애플리케이션 내에서 URL을 변경할 수 있도록 하는 컴포넌트
    // - <Link> 컴ㅍ포넌트는 새로운 요청을 보내지 않음
    // - 외부 페이지로 이동하고자 할땐 a 요소를 사용해야함
    // - 컴포넌트 클릭 시 다른 작업을 같이 수행한다면 해당 작업이 정상적으로 완료되지 않을 수 있음
    // - 특정 작업 결과에 따라 다른 경로로 이동하고 싶을 때 
    const onClick = () => {
        // for (let index = 0; < 100000; index++) {
    ...
    return (
        <div>
            ...
            <br />
            <Link to='/component' onClick={onClick}>링크 컴폰넌트</Link>
            <br />
